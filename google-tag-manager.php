...
class GoogleTagManagerPlugin extends Plugin
{
    ...
    public function onAssetsInitialized() {
        ...
        $auth = trim($this->grav['config']->get('plugins.google-tag-manager.auth', ''));
        $env = trim($this->grav['config']->get('plugins.google-tag-manager.env', ''));

        if (!$this->isTheRightDomain()) return;

        // Embed Google Tag Manager script
        $this->grav['assets']->addInlineJs($this->generateTrackingCode($trackingId, $auth, $env), null);
    }
    /**
     * Generate GTM Code for embeded.
     * @param string $trackingId Google Tag Manager Tracking ID
     * @param string $auth Google Tag Manager authetification
     * @param string $env Google Tag Manager Environment
     */  
    private function generateTrackingCode($trackingId, $auth, $env) {
        $code = <<<EOF
<!-- Google Tag Manager -->
